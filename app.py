...
# ==============================
def _join_model_parts_if_needed():
    """Join split model parts into full model if it doesn't exist."""
    full_path = "plant_disease_final_model.keras"
    if os.path.exists(full_path):
        return
    parts = []
    i = 1
    while os.path.exists(f"plant_disease_model_part{i}.bin"):
        parts.append(f"plant_disease_model_part{i}.bin")
        i += 1
    if not parts:
        raise FileNotFoundError(
            "Model not found. Either place plant_disease_final_model.keras in the project folder, "
            "or run split_model.py first and ensure plant_disease_model_part*.bin files exist."
        )
